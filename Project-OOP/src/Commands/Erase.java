    ...
    public String execute() throws TransformerException {
        ...
        try {
            if (context.getCommandOptions()[1].chars().allMatch(Character::isDigit)) {
                context.getShapes().get(Integer.parseInt(context.getCommandOptions()[1]) - 1).removeNODE();
                context.getShapes().remove(Integer.parseInt(context.getCommandOptions()[1]) - 1);
